...
import { uploadFile, convertEmojiToUnicode } from '../utils/tool';
import EmojiSelector from './EmojiSelector';
class ChatItem extends Component {
  state: any = {};
  initAnit: any[] = [...Array(3).fill(0)].map(() => new Animated.Value(1));
    ...
    this.setState(
      ...
      async () => {
        if (this.state.isPlayIng) {
          ...
          const { durationMillis } = await this.state.sound.playAsync();
  ...
  render() {
    ...
    const status = item.audioStatus ? JSON.parse(item.audioStatus) : null;
            ...
            >
              {item.file && isAudio && !isImage ? (
                ...
                >
                  ...
                  >
                    {status && Math.floor(status.durationMillis / 1000) + '"'}
                  </Box>
                  {this.state.rssArr.map((val: any, index: any) => (
                    <Animated.Text
                      key={index}
                      style={{
                        opacity: this.state.rssArr[2 - index],
                ...
                <Image
                  source={'data:image/jpeg;base64,' + item.file}
                  ...
                  {this.state.rssArr.map((val: any, index: any) => (
                    <Animated.Text
                      ...
                      style={{
                        ...
                        transform: [{ scale: (3 - index) / 3 }],
                        fontSize:
                          index === this.state.rssArr.length - 1 ? 16 : 14,
                        fontWeight:
                          index === this.state.rssArr.length - 1 ? 800 : 400,
                        color: '#fff',
                      }}
                    >
                      {' '}
...
export class Chats extends PureComponent {
  ...
  constructor(props: any) {
    ...
    const { setRouteName, setMsgLen, msgLen, userInfo } = useMobxStore;
    ...
    this.state = {
      ...
      animatedHeights: [...Array(20).fill(0)].map(() => new Animated.Value(4)),
      audioIndex: 0,
  ...
  recordAd = async () => {
    try {
      ...
      this.state.recording.setOnRecordingStatusUpdate(this.updateWaveform);
      ...
      () => {
        if (this.state.audioIndex >= 20) {
          ...
          waState.map((value: any, index: any | number) => {
            Animated.timing(this.state.animatedHeights[index], {
              toValue: value,
              duration: 300,
              useNativeDriver: true,
  ...
  render() {
    ...
    return (
      <Box flex="1" bg="#fff">
        ...
        >
          <Box flex="1" bg="#fff">
            <Animated.View style={[styles.audioAnimated, { opacity, bottom }]}>
              <Image
            ...
            <FlashList
              data={allText}
              ref={this.listRef}
              keyExtractor={(item: any, index: number): any => index}
          ...
          <Box bg="#fff" height="100" position="relative">
            ...
            >
              ...
              >
                <FontAwesome name="microphone" color="#fff" size={15} />
              </TouchableOpacity>
                  ...
                  {this.state.showBo ? (
                    <HStack justifyContent="center" alignItems="center">
                      {this.state.animatedHeights.map(
                        (value: number, index: any | number | bigint) => {
                          const scale = this.state.animatedHeights[index];
                          const width = 2;
              ...
              >
                <AntDesign name="smile-circle" color="#fff" size={16} />
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.send_button}
              ...
              >
                <FontAwesome
                  name={this.state.isFoucs ? 'send' : 'plus'}
                  color="#fff"
                  size={15}
                  style={{
          ...
          {this.state.showOptions ? (
            <MoreOptions uploadPicture={this.uploadPicture}></MoreOptions>
          ) : (
            <></>
          )}
        </KeyboardAvoidingView>
