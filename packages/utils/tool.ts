...
export const getWaveformData = (status: {
  ...
  for (let i = 0; i < status.durationMillis; i += step) {
    const amplitude = status?.metering?.amplitude?.[0] / maxAmplitude || 0;
    const y = amplitude * 50;
    waveformData.push(`${i},${y}`);
  }
  return waveformData;
};
export const UploadRequest = async (url: string, params: any) => {
  try {
    ...
    const { body } = await FileSystem.uploadAsync(`${BaseUrl}${url}`, path, {
      fieldName: 'file',
      httpMethod: 'POST',
...
export const convertEmojiToUnicode = (text: string) => {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (char.charCodeAt(0) >= 55296 && char.charCodeAt(0) <= 56319) {
      ...
      if (i < text.length - 1) {
        ...
        if (
          ...
          const codePoint =
            ...
            (nextChar.charCodeAt(0) - 56320) +
            65536;
          unicodeText += '\\u' + codePoint.toString(16) + ';';
          i++;
          continue;
        }
      }
